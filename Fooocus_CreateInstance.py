...
parser = argparse.ArgumentParser("Vastai create Foocus instance")
parser.add_argument('civit_token', type=str, help="Civit.ai token for provisioning scripts", default="")
parser.add_argument('hf_token', type=str, help="Huggingface token for provisioning scripts", default="")
...
def copyCloudData(instance):
    ...
    client.cloud_copy(src='/workspace/ComfyUI/models/checkpoints/current', dst='/opt/Fooocus/models/checkpoints',instance=instance.get('id'), connection='13808', transfer="Cloud To Instance")
    ...
    client.cloud_copy(src='/workspace/ComfyUI/models/loras/current', dst='/opt/Fooocus/models/loras',instance=instance.get('id'), connection=cloud_data_connection_id, transfer="Cloud To Instance")

def create_contract():
    query = F'dph>={dph} reliability>0.95 num_gpus=1 verified=true gpu_ram>={gpu_ram} disk_space>=100 gpu_name in {gpu_names} geolocation in [DE, FR, PL, SE, ES, PT, CH, NO, LU, CZ, US]'
    #print(query)
